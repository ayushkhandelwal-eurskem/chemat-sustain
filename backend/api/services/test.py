...
from ..schemas.test import TestCreate, TestUpdate, TestListings, ListingsResponse, TestResponse
import json
...
class TestService:
    ...
    async def create_test(self, test_data: TestCreate) -> Test:
        ...
        if existing_test:
            raise HTTPException(
                ...
                detail=f"Test with name '{test_data.test_name}' for work package '{test_data.work_package_name}' and element '{test_data.element_cms_id}' already exists"
            )
    ...
    async def update_test(self, test_id: int, test_data: TestUpdate) -> Test:
        ...
        if any(field in update_data for field in combination_fields):
            ...
            # Check if this combination already exists (excluding current test)
            stmt = select(Test).filter(
            ...
            if existing_test:
                raise HTTPException(
                    status_code=status.HTTP_409_CONFLICT,
                    detail=f"Test with name '{new_test_name}' for work package '{new_work_package}' and element '{new_element_cms_id}' already exists"
                )
    ...
    async def get_public_tests(self, skip: int = 0, limit: int = 100) -> tuple[List[Test], int]:
    ...
    async def bulk_update_release_flags(
        ...
        if len(tests) != len(test_ids):
            ...
            missing_ids = [id for id in test_ids if id not in found_ids]
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
    ...
    async def get_listings(self, request: TestListings, is_private_user: bool):
        ...
        if request.work_package_name == None and request.element_cms_id == None and request.test_name == None:
        ...
        elif request.work_package_name != None and request.element_cms_id == None and request.test_name == None:
            result = await self.db.execute(
