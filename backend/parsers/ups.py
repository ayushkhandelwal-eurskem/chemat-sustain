...
class TreatmentData:
    ...
    photon: Optional[str] = None
    analyser: Optional[str] = None
    pass_energy_ev: Optional[str] = None
    bias_v: Optional[Union[int, float, str]] = None
...
@dataclass
class UPSFinalResultBlock:
    sample_identifier: Optional[str] = None
    endpoint: Optional[str] = None
    spot: Optional[str] = None
    work_function: Optional[Union[str, float]] = None
    fermi_present: Optional[str] = None
    vb_offset: Optional[Union[int, float, str]] = None
...
class UPSParser:
    def __init__(self, file_path: str, sheet_name: str = "Test_conditions"):
        ...
        try:
            ...
            logger.info("Successfully loaded UPS workbook: %s", file_path)
    ...
    def _metric_name_from_sheet(self, sheet_name: str, title: Optional[str]) -> str:
        ...
        if "seco" in source or "sec" in source:
            return "Secondary Electron Cut-Off (SECO)"
        if "vbheii" in source or "vb he ii" in source or "he ii" in source or "vbh" in source:
            return "Valence Band (VB) with He II photon"
        if re.search(r"\bvb\b", source):
            return "Valence Band (VB)"
        return "Long Range Spectrum"

    def _find_matching_processed_sheet(self, raw_sheet_name: str) -> Optional[str]:
        raw_lower = raw_sheet_name.lower()

        candidates = {
            "all": "Processed data_UPS_AuNPs1aU_ALL",
            "seco": "Processed data_UPS_AuNPs1aU_SEC",
            "sec": "Processed data_UPS_AuNPs1aU_SEC",
            "vbheii": "Processed data_UPS_AuNPs1aU_VBH",
            "vbh": "Processed data_UPS_AuNPs1aU_VBH",
            "vb": "Processed data_UPS_AuNPs1aU_VB",
        }
    ...
    def extract_treatment_data(self) -> Dict[str, Any]:
        ...
        result = TreatmentData(
            facility=ws.cell(row=57, column=1).value,
            photon=ws.cell(row=58, column=2).value,
            analyser=ws.cell(row=59, column=2).value,
            pass_energy_ev=ws.cell(row=60, column=2).value,
            bias_v=ws.cell(row=61, column=2).value,
            slit=ws.cell(row=62, column=2).value,
    ...
    def _extract_raw_block(self, raw_sheet_name: str, processed_sheet_name: Optional[str]) -> Dict[str, Any]:
        ...
        kinetic_vals: List[float] = []
        intensity_vals: List[float] = []

        # expected numeric data: col 3 = kinetic energy, col 4 = intensity
        for row_idx in range(3, ws.max_row + 1):
            ke = self._safe_float(ws.cell(row=row_idx, column=3).value)
            intensity = self._safe_float(ws.cell(row=row_idx, column=4).value)

            if ke is not None and intensity is not None:
                spectrum_points.append(asdict(UPSSpectrumPoint(
                    kinetic_energy_ev=ke,
                    intensity_counts_per_s=intensity,
                )))
                kinetic_vals.append(ke)
                intensity_vals.append(intensity)

        assay_notes = self._extract_notes_after_marker(ws)

        result = UPSRawDataBlock(
            ...
            kinetic_energy_unit="eV",
            intensity_unit="counts per s",
            min_kinetic_energy_ev=min(kinetic_vals) if kinetic_vals else None,
            max_kinetic_energy_ev=max(kinetic_vals) if kinetic_vals else None,
    ...
    def extract_processed_data(self, raw_blocks: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for raw_block in raw_blocks:
            ...
            blocks.append(asdict(UPSProcessedDataBlock(
    ...
    def extract_final_results(self) -> List[Dict[str, Any]]:
        ...
        endpoint = ws.cell(row=1, column=2).value

        results: List[Dict[str, Any]] = []

        for row_idx in range(4, ws.max_row + 1):
            spot = ws.cell(row=row_idx, column=2).value
            work_function = ws.cell(row=row_idx, column=3).value
            fermi_present = ws.cell(row=row_idx, column=4).value
            vb_offset = ws.cell(row=row_idx, column=5).value

            if all(v in (None, "") for v in [spot, work_function, fermi_present, vb_offset]):
                continue

            results.append(asdict(UPSFinalResultBlock(
                sample_identifier=sample_identifier,
                endpoint=endpoint,
                spot=spot,
                work_function=work_function,
                fermi_present=fermi_present,
                vb_offset=vb_offset,
            )))
    ...
    def extract_statistical_analysis(self) -> Dict[str, Any]:
        return {
            "available": False,
            "notes": "No dedicated statistical analysis section was found in this UPS workbook.",
        }
    ...
    def parse_all_data(self) -> Dict[str, Union[Dict, List]]:
        try:
            ...
            logger.info("FINAL UPS JSON:\n%s", json.dumps(parsed_data, indent=2, default=str))
            return parsed_data

        except Exception as e:
            logger.error("Error parsing UPS file: %s\n%s", e, traceback.format_exc())
            raise


def parse_excel_ups(file_path: str, sheet_name: str = "Test_conditions") -> Dict[str, Union[Dict, List]]:
    try:
        parser = UPSParser(file_path, sheet_name)
        return parser.parse_all_data()
    except Exception as e:
        logger.error("Error in parse_excel_ups: %s\n%s", e, traceback.format_exc())
        raise


if __name__ == "__main__":
    file_path = "/Users/ayushkhandelwal/Documents/chemat-sustain/backend/data/CMS_WP4_1a_AuNP_UPS.xlsx"
    parsed_data = parse_excel_ups(file_path)

    print("=" * 70)
    print("UPS PARSER OUTPUT SUMMARY")
    print("=" * 70)
