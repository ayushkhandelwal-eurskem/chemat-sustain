...
// --- Raw Data (parser top-level "replications") ---
...
// --- Full API Response (matched to actual DB/API shape) ---
interface ROSData {
  test_details: {
    ...
    <>
      {downloadFilename && (
        <div className="flex justify-end mb-3">
          <button
            onClick={() => downloadTable(id, downloadFilename)}
...
/* ================================================================
   Helper: CSV download for any table by ID
   ================================================================ */
...
const ROSDataViewer: FC<PageProps> = ({ work_package, element, test }) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.log("Fetched ROS data:", response.data);
  ...
  /* ---------- Safe data accessors (handle both API shapes) ---------- */
  const safeRawBlocks: ROSRawDataBlock[] = useMemo(() => {
    return (data?.raw_data ?? data?.replications ?? []) as ROSRawDataBlock[];
  }, [data?.raw_data, data?.replications]);

  const safeMeanBlocks: ROSMeanDataBlock[] = useMemo(() => {
    return data?.final_results?.mean_data ?? [];
  }, [data?.final_results]);

  /* ---------- Chart Data for Raw Data ---------- */
  const rawChartData = useMemo(() => {
    if (!safeRawBlocks?.[selectedRawBlock]) return [];
    ...
    return block.concentrations.map((conc, ci) => {
      ...
      block.experiments.forEach((exp) => {
        point[exp.experiment_label ?? `Exp ${ci}`] = exp.values[ci];
      });
      return point;
    });
  }, [safeRawBlocks, selectedRawBlock]);

  /* ---------- Chart Data for Processed Data ---------- */
  const makeAnalysisChartData = useCallback(
    (block: ROSDataAnalysisBlock | undefined) => {
      if (!block) return [];
      return block.concentrations.map((conc, ci) => {
        const point: Record<string, any> = { concentration: String(conc) };
        block.experiments.forEach((exp) => {
                ...
                rows={[
                  ...
                  { label: "Vial", value: mat.vial },
                  {
                    ...
                    value: mat.preparation_date
                      ? String(mat.preparation_date).match(/^\d{4}-\d{2}-\d{2}/)?.[0] ?? mat.preparation_date
                      : null,
                  },
            ...
            <CollapsibleSection title="Treatment Data">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div className="bg-green-50 p-3 rounded-md">
                  ...
                  <p className="text-sm">
                    {treat.concentration.positive_control_abbr ?? "N/A"} &mdash;{" "}
                ...
                <div className="bg-red-50 p-3 rounded-md">
                  ...
                  <p className="text-sm">
                    {treat.concentration.negative_control_abbr ?? "N/A"} &mdash;{" "}
            ...
            <div className="flex justify-between items-center mb-6">
              ...
              <button
                onClick={() =>
                  downloadTableCSV(
                    "rawDataTable",
                    `ROS_Raw_Block_${selectedRawBlock + 1}`
                  )
                }
                  ...
                  <div className="mb-8">
                    ...
                    <ResponsiveContainer width="100%" height={400}>
                      ...
                      >
                        ...
                        <Tooltip />
                        <Legend />
                        {safeRawBlocks[selectedRawBlock].experiments.map(
                          (exp, i) => (
                            <Bar
                              key={exp.experiment_label ?? i}
                              dataKey={exp.experiment_label ?? `Exp ${i}`}
                  ...
                  <table id="rawDataTable" className="min-w-full bg-white border border-gray-200 text-sm">
                    <thead>
                      <tr className="bg-gray-100">
                        ...
                        {safeRawBlocks[selectedRawBlock].concentrations.map(
                          (c, i) => (
                            <th key={i} className="py-2 px-3 border text-center">
                              {c}
                            </th>
                    ...
                    <tbody>
                      {safeRawBlocks[selectedRawBlock].experiments.map(
                        (exp, ei) => (
                          <tr
                            key={ei}
                            className={ei % 2 === 0 ? "bg-gray-50" : ""}
                          >
                            <td className="py-2 px-3 border font-medium">
                              {exp.experiment_label ?? "N/A"}
                            </td>
                            <td className="py-2 px-3 border">
                              {exp.cytometric_events ?? "N/A"}
                            </td>
                            {exp.values.map((v, vi) => (
              ...
              >
                <div className="mb-4 flex items-center gap-4 flex-wrap">
                  ...
                  <span className="text-sm">
                    ...
                    <AcceptanceBadge
                      status={data.processed_data.fluorescence_sum.cv_acceptance}
                    />
                  </span>
                  <span className="text-sm">
                  ...
                  <div className="mb-6">
                    <ResponsiveContainer width="100%" height={350}>
                      ...
                      >
                        ...
                        <Tooltip />
                        <Legend />
                        {data.processed_data.fluorescence_sum.experiments.map(
                          (exp, i) => (
                            <Line
                              key={exp.label}
                              type="monotone"
              ...
              >
                <div className="mb-4 flex items-center gap-4 flex-wrap">
                  ...
                  <span className="text-sm">
                    <span className="font-semibold">CV Acceptance:</span>{" "}
                    <AcceptanceBadge
                      status={data.processed_data.percentage_high_ros.cv_acceptance}
                    />
                  </span>
                  <span className="text-sm">
                    <span className="font-semibold">Cytometric Events:</span>{" "}
                  ...
                  <div className="mb-6">
                    <ResponsiveContainer width="100%" height={350}>
                      ...
                      >
                        ...
                        <Tooltip />
                        <Legend />
                        {data.processed_data.percentage_high_ros.experiments.map(
                          (exp, i) => (
                            <Line
                              key={exp.label}
                              type="monotone"
              ...
              data.processed_data.experiment_5_separate.concentrations.length > 0 && (
                <CollapsibleSection title="Experiment 5 (Separate)" defaultOpen={false}>
                  <div className="flex justify-end mb-3">
                    <button
                      onClick={() =>
                        downloadTableCSV("experiment5Table", "ROS_Experiment_5")
                      }
                      className="flex items-center gap-1 bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 transition text-sm"
                    >
                      <Download size={14} />
                  ...
                  <div className="overflow-x-auto">
                    ...
                    >
                      ...
                      <tbody>
                        {data.processed_data.experiment_5_separate.concentrations.map(
                          (c, i) => (
                            <tr key={i} className={i % 2 === 0 ? "bg-gray-50" : ""}>
                              ...
                              <td className="py-2 px-3 border">
                                {data.processed_data.experiment_5_separate.values[i] != null
                                  ? Number(
                                      data.processed_data.experiment_5_separate.values[i]
                                    ).toFixed(2)
                                  : "-"}
                              </td>
                              <td className="py-2 px-3 border">
            ...
            <div className="flex justify-between items-center mb-6">
              ...
              <button
                onClick={() =>
                  downloadTableCSV(
                    "meanDataTable",
                    `ROS_Mean_Block_${selectedMeanBlock + 1}`
                  )
                }
                className="flex items-center gap-1 bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 transition"
              >
                                ...
                                {block.concentrations.map((c, ci) => (
                                  ...
                                  >
                                    {exp.values[String(c)] != null
                                      ? Number(exp.values[String(c)]).toFixed(2)
                                      : "-"}
                                  </td>
                                ))}
                              </tr>
...
/* ================================================================
   Sub-component: Analysis Data Table (Processed Data)
   ================================================================ */
...
/* ================================================================
   Sub-component: ANOVA + Post-Hoc Section
   ================================================================ */
...
const ANOVASection: FC<{
  ...
  return (
    <>
      <CollapsibleSection
        title={`ANOVA: ${anova.metric_name ?? "Unknown Metric"}`}
      >
        {/* Significance badge */}
        <div className="mb-4 flex items-center gap-4 flex-wrap">
          <span className="text-sm">
            ...
            <AcceptanceBadge
              status={anova.p_value_significant ? "YES" : "NO"}
            />
          </span>
          <span className="text-sm">
            ...
            <tbody>
              {anova.summary.map((row, i) => (
                <tr key={i} className={i % 2 === 0 ? "bg-gray-50" : ""}>
                  ...
                  <td className="py-2 px-3 border text-right">
                    {row.wariancja != null
                      ? Number(row.wariancja).toFixed(4)
                      : "-"}
                  </td>
                </tr>
              ))}
            ...
            <tbody>
              {anova.anova_table.map((row, i) => (
                <tr key={i} className={i % 2 === 0 ? "bg-gray-50" : ""}>
                  ...
                  <td className="py-2 px-3 border text-right">
                    {row.f_critical != null
                      ? Number(row.f_critical).toFixed(4)
                      : "-"}
                  </td>
                </tr>
              ))}
              ...
              <tr className="bg-blue-50 font-semibold">
                ...
                <td className="py-2 px-3 border text-right">
                  {anova.total_ss != null
                    ? Number(anova.total_ss).toFixed(4)
                    : "-"}
                </td>
                <td className="py-2 px-3 border text-right">
                  {anova.total_df ?? "-"}
